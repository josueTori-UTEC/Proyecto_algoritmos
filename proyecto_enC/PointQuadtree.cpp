...
PointQuadtree::PointQuadtree(Rectangle _boundary)
...
bool PointQuadtree::insert(Point* p) {
...
void PointQuadtree::query(Rectangle* range, std::vector<Point*>& found) const {
...
bool PointQuadtree::remove(Point* p) {
    if (!boundary.contains(p)) return false;
